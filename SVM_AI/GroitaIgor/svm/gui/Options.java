...
public class Options extends Dialog implements AdjustmentListener{
	...
	Label eta_label, C_label;
	TextField eta_tf, C_tf; 	
	...
	public Options(SVM svm){
		...
		y+=30;
		C_label = new Label("C: ");
		C_label.setBounds(25,y,100,20);
		C_label.setForeground(Color.white);
		add(C_label);
		C_tf = new TextField("");
		C_tf.setForeground(Color.black);
		C_tf.setBounds(200,y,90,20);
		add(C_tf);
		C_tf.setText(""+svm.settings.C);
	...
	public boolean handleEvent(Event e){
		if(e.id==Event.WINDOW_DESTROY){
			...
			float C = svm.settings.C_default;
			try{
				C = Float.parseFloat(C_tf.getText());
			}
			catch(NumberFormatException nfe){}
			svm.settings.C = C;
