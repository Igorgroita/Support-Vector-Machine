...
public class MaxMarginIPRaluca extends Algorithm{
	SVM svm;
	
	public MaxMarginIPRaluca(SVM svm){
		super(svm);
		this.svm = svm;
		if(svm.ind.V != null){
			name = "Max Margin - Interior Point Method - Raluca";
	...
	public void run(){
		...
		float[][] I_n = new float[dim][dim];
		float[][] v = new float[dim][1];
		float[][] x = new float[dim][1];
		float[][] y = new float[N][1];
		float[][] lambda = new float[N][1];
		float[][] sol = new float[dim][1];
		float[] w = new float[dim];
		float[] w_star = new float[dim];
		float[] w_final = new float[dim+1];
		float[] w_plus = new float[dim+1];
		float[] w_minus = new float[dim+1];
		float tolerance = 1e-5f;
		float value;
		
		float globalMinimum = Float.MAX_VALUE;
		
	
		// Maximum of norma 
		for(i = 0; i < N; i++){
			norma = (float)Math.sqrt(dotProduct(svm.ind.V[i].X, svm.ind.V[i].X));
			if (r <= norma)
				r = norma;
		}
		...
		for (i = 0; i < N; i++){
			...
			for (j = 0; j < dim; j++)
				X[i][j] = y_changed_i * svm.ind.V[i].X[j];
		}
		
		// Creating the identity matrix
		for(i = 0; i < dim; i++)
			for(j = 0; j < dim; j++)
				I_n[i][j] = (i == j) ? 1.0f : 0.0f;
			
		// Initializing the v matrix with 0
		for(i = 0; i < dim; i++)
			v[i][0] = 0;
		
		// Creating the x matrix with 0
		for(i = 0; i < dim; i++)
			x[i][0] = 0;
		
		// Creating the y matrix with 1
		for(i = 0; i < N; i++)
			y[i][0] = 1;
		...
		for(b = -q; b <= q; b ++){
			...
			for(i = 0; i < N; i++){
				y_changed_i = svm.ind.V[i].cl.Y == 0? -1 : 1;
				c[i][0] = 1 + y_changed_i * b;
			}
			
			// Searching for the minimum value of the function
			
			value = InteriorPoint(I_n, v, X, c, x, y, lambda);
			...
			// Printing the minimum values of the function with respect to b
		...
		for(i = 0; i < N; i++){
			y_changed_i = svm.ind.V[i].cl.Y == 0? -1 : 1;
			c[i][0] = 1 + y_changed_i * b_star;
		}		
		sol = IPM(I_n, v, X, c, x, y, lambda);
		for(i = 0; i < dim; i++) w[i] = sol[i][0];
		...
		*/
		...
		w_final[dim] = w_final[dim] - b_star;
		w_plus[dim] = w_plus[dim] - b_star + 1;
		w_minus[dim] = w_minus[dim] - b_star - 1;
		
		if((float)Math.sqrt(dotProduct(w, w)) > q/r){
			svm.control.ta.append("A fost incalcata constrangerea de marginire!");
			svm.design.calculates = false;
			svm.design.repaint();
		}
		else{
			//svm.control.ta.append("Stage " + b_star + ": Solution found");
			svm.outd.computing_time = System.currentTimeMillis() - t;
			
			float[] bb = Tools.translate(w_final,svm.ind.V);
			w_final[dim] = -bb[2];
			float[] w0 = new float[dim+1];
			for(j=0; j<dim; j++) w0[j] = w_final[j];
			w0[dim] = -bb[0];
			float[] w1 = new float[dim+1];
			for(j=0; j<dim; j++) w1[j] = w_final[j];
			w1[dim] = -bb[1];			
			if(dim==2) svm.design.setPointsOfMaxLine(w_final,w0,w1);			
			
			svm.outd.w = w_final;
			svm.outd.accuracy = getAccuracy(w_final);
			svm.outd.margin = bb[3];
			svm.outd.showInputData();
		...
		/*
		if (dim == 2){
            svm.design.setPointsOfLine(w_final);
			//svm.design.setPointsOfLinePlus(w_plus); 
			//svm.design.setPointsOfLineMinus(w_minus);
			}
		*/
